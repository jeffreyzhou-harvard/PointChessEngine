"""UCI adapter for the RLM-inspired engine."""
...
from .engine import DEFAULT_MOVETIME_MS, RLMChessEngine, SearchLimits


class RLMUCI:
    def __init__(self) -> None:
        self.board = chess.Board()
        self.engine = RLMChessEngine()
        self.skill = 20
    ...
    def run(self, input_stream: TextIO = sys.stdin, output_stream: TextIO = sys.stdout) -> None:
        for raw_line in input_stream:
            ...
            if not line:
                continue
            if line == "uci":
                self._write(output_stream, "id name PointChess RLM")
...
def run_uci(input_stream: TextIO = sys.stdin, output_stream: TextIO = sys.stdout) -> None:
    RLMUCI().run(input_stream=input_stream, output_stream=output_stream)

"""CLI entry point: ``python -m methodologies.langgraph``.

Examples
--------

    # Run the orchestrator with defaults (claude-sonnet-4-5 -> ./engines/langgraph)
    python -m methodologies.langgraph

    # Custom output directory and a stronger model with one rework pass
    python -m methodologies.langgraph --output ./out --model claude-opus-4-5 --revisions 1

    # Pass extra context for the Context Analyst to assess
    python -m methodologies.langgraph --context https://github.com/lichess-org/scalachess
...
from methodologies.langgraph.runner import (
    DEFAULT_MODEL,
    DEFAULT_OUTPUT_DIR,
    _load_env_key,
    default_brief,
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m methodologies.langgraph",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        from methodologies.langgraph import build_graph  # noqa: F401  (import-time check)
        from methodologies.langgraph.runner import _make_llm

...
```bash
# Web UI (default)
python -m oneshot_react_engine
# -> http://127.0.0.1:8000

# UCI mode (for chess GUIs)
python -m oneshot_react_engine --uci

# Custom host/port
python -m oneshot_react_engine --host 0.0.0.0 --port 9000

# Tests
pip install -r oneshot_react_engine/requirements.txt
python -m pytest oneshot_react_engine/tests/ -v
```
...
`python -m oneshot_react_engine --uci`, working directory = the repo root.

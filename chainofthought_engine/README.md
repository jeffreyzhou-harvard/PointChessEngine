...
├── __init__.py
├── __main__.py                 # `python -m chainofthought_engine [--uci|--ui]`
├── README.md
├── requirements.txt
...
The always-on suite is **`pytest chainofthought_engine/tests/`**
...
`python -m chainofthought_engine` (or `--ui`) starts a tiny
...
`python -m chainofthought_engine --uci` exposes the engine as a UCI
...
`chainofthought_engine/search/elo.py` for the full rationale and
tuning notes.
...
```bash
# one-time install
pip install -r chainofthought_engine/requirements.txt

# run all tests
python -m pytest chainofthought_engine/tests/ -v
```

## Running the program

```bash
python -m chainofthought_engine --uci             # UCI mode (stdin/stdout)
python -m chainofthought_engine --ui              # browser UI on :8000
python -m chainofthought_engine --port 9999       # browser UI on :9999
python -m chainofthought_engine                   # default: --ui on :8000
```

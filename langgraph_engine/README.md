...
├── __main__.py            # CLI: python -m langgraph_engine
...
.venv/bin/pip install -r langgraph_engine/requirements.txt
```
...
```bash
# default: claude-sonnet-4-5, output ./langgraph_output, 1 rework pass
python -m langgraph_engine

# heavier model, two rework passes, custom output
python -m langgraph_engine --model claude-opus-4-5 --revisions 2 --output ./out
...
```bash
.venv/bin/python -m pytest langgraph_engine/tests/ -q
# 53 passed in <1s
```
